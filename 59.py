    ...
    for k2 in range(97, 123):
        for k3 in range(97, 123):
            ...
            for l in encrypted:
                ...
                d = l ^ k
                sum_of_values += d
                if (65 <= d <= 90) or (97 <= d <= 122) or d == 32:
                   decrypted.append(chr(d))

            decrypted_all.append(["".join(decrypted), chr(k1) + chr(k2) + chr(k3), sum_of_values])
            decrypted = []
            sum_of_values = 0

for dks in decrypted_all:
    d = dks[0]
    k = dks[1]
    s = dks[2]
    if max(set(d), key=d.count) == " " or max(set(d), key=d.count) == "e":
        if "the" in d or "The" in d:
            print(d, k, s)

# Then just search that butt-load of text manually HINT: the key is "god"
