...
def find_palin(n, i=50, first=False):
    if i == 0: return False

    if str(n) == str(n)[::-1] and not first:
        return n

    r = int("".join(reversed(list(str(n)))))

    return find_palin(n+r, i=i-1)

result = 0

for p in range(1,10001):
    if not find_palin(p, first=True):
        result += 1
